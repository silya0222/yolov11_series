...
from ultralytics.nn.modules.common_modules import (C2f_SE,SwinTransformerBlock,ASFF)
import torch
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if isinstance(f, list):
                print(f"DEBUG: 正在处理模块 {m}, 这里的输入 f 是一个列表: {f}")
                print("如果这个模块是 SimAM/Conv/C3k2，它不支持列表输入，请检查 YAML 文件的 from 列！")
            # === [添加调试打印结束] ===
            c1, c2 = ch[f], args[0]
            ...
            c1, c2 = ch[f], args[0]
            if c2 != c1:
                # SimAM 不改变通道数，如果 yaml 写的输出通道不对，强制让它等于输入
                c2 = c1
            args = [c1, c2, *args[1:]]
