...
def plot_pr_comparison():
    ...
    if path_baseline:
        ...
        files_map["Baseline"] = {"path": path_baseline, "color": COLOR_BASELINE, "style": CONFIG_LINE_STYLE}
    else:
        print(f"❌ 失败: 找不到 Baseline 文件 ({name_baseline})")

    if path_improve:
        print(f"✅ 成功找到 Improve 文件: {path_improve}")
        files_map["Improve"] = {"path": path_improve, "color": COLOR_IMPROVE, "style": CONFIG_LINE_STYLE}
    else:
